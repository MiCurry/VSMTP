 ...
 *      Author: coanj
 *
 *	This program connects to otp_dec_d, and asks it to reverse the one-time pad style
 *	encryption that was performed by otp_enc_d on the text, returning plaintext.
 *	By itself, otp_dec will not decrypt.
 *
...
int main( int argc, char *argv[ ] ) {
	...
	if( argc<4 ) error( "Usage: otp_dec ciphertext key port\n");
	...
	while(index != charCount){
		ch = fgetc(fp);
		//	Make sure characters are alphabetic
		if(!isalpha(ch)){
			if(ch!=' '){
