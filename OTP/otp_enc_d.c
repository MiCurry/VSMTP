 ...
 * 	encoding. This program will listen on a particular port, assigned when it is first ran, and
 * 	receives plain text and a key via that port when a connection to it is made. It will then write
 * 	back the cipher text to the process that it is connected to via the same port. Note that the key
 * 	passed in must be at least as big as the plain text.
 *
...
int main( int argc, char* argv[ ] ) {
	...
	while( 1 ) {
		...
		while(size--){
			...
			//	encode using one time pad method
			cipher = ( textConvert + keyConvert ) % 27;
			if(cipher<0) cipher += 27;
			
			//	new encrypted textFile
